...
/**
 * A server program that returns stock price upon client request. When
 * a client connects, a new thread is started to handle it. Receiving client data,
 * proceed the input, and sending the response back is all done on the thread, allowing
...
public class StockPriceServer {
    ...
    public static void main(String[] args) throws Exception {
		// 1a. create a server socket listening at port 59898
        try (ServerSocket listener = new ServerSocket(59898)) {
            System.out.println("The stock price server is running...");
            // 1a. create ExecutorService object of 20 threads (Check static method Executors.newFixedThreadPool)
            ExecutorService pool = Executors.newFixedThreadPool(20);
            // 1b. have the server keep listening the coming socket requests
            /* Use [server socket].accept() to establish connection with client socket.
             * Use [ExecutorService].execute() to start a new thread
             * It is advised to pass the connected socket to an inner class and 
             * let the inner class handles the server-client interaction 
             * for a more intuitive architecture .
             * 
             */
            while (true) {
    ...
    private static class StockChecker implements Runnable {
        ...
        //  2a. Use data structure to map the stock code and the corresponding prices.
        /*
         * One of the approach is to use Map<String, Double>
         * Example data:
         * AAPL: 257.13
         * FB: 194.32
         * MSFT: 144.46
        ...
        public void run() {
            ...
            try {
            	// 2b. use BufferedReader, InputStreamReader to handle socket input stream (output to clients)
            	BufferedReader in = new BufferedReader(new InputStreamReader(socket.getInputStream()));
            	// 2c. use PrintWriter to handle socket output stream (input from clients)
            	PrintWriter out = new PrintWriter(socket.getOutputStream(), true);
                String q = "";
                
                System.out.println("Server: Waiting for input..");
                
                // 2c. inside the while loop, read the stock code from the clients and return the corresponding price to the client
                /*
                 * use try..catch to handle potential error (eg. IOException, undefined stock code)
                 */
				 
				// 2d. Implement logic to terminate the connection. (eg. when receiving 'exit' msg)
                while (!q.equals("EXIT")) {
